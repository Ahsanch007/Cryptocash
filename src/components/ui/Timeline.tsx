...
const Timeline: React.FC = () => {
  const [activeIndex, setActiveIndex] = useState<number>(0); // Start with the first item as active
  const [itemsToShow, setItemsToShow] = useState<number>(5); // Default value for larger screens
  ...
  return (
    <div className="relative bg-gradient-to-r from-blue-800 to-blue-900 text-white py-12 px-4">
      ...
      <button
        ...
        className={`absolute top-[33%] sm:top-[34%] z-[1] left-0 transform -translate-y-1/2 p-2 rounded-full ${
          activeIndex === 0
      ...
      <button
        ...
        className={`absolute top-[33%] sm:top-[34%] z-[1] right-0 transform -translate-y-1/2 p-2 rounded-full ${
          activeIndex >= timelineData.length - itemsToShow
            ? "bg-gray-500 cursor-not-allowed"
            : "bg-white text-[#0e082c] hover:bg-[#ff69c9] hover:text-white shadow-md"
        }`}
      ...
      <div className="relative flex items-center justify-between">
        ...
        <div
          className={`absolute top-[19%] sm:top-[17%] ${
            activeIndex === 3 ? "w-[22%]" : activeIndex ? "w-[100%]" : "w-[100%]"
          }  lg:w-[70%] left-0 h-1 bg-[#ff69c9] transform -translate-y-1/2 transition-all duration-300`}
        ></div>
            ...
            >
              <div
                className={`w-[20px] h-[20px] rounded-full ${
                  index + activeIndex <= activeIndex ? "bg-white" : "bg-white"
                }`}
              ></div>
            </div>
